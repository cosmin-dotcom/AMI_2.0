...
class DenseGaussianProcessregressor:
    ...
    def build_model(self, X: NDArray[NDArray[np.float_]], y: NDArray[np.float_]) -> gpflow.models.GPR:
        ...
        data=(X, y), 
        kernel=gpflow.kernels.Sum(RBF(lengthscales=np.ones(X.shape[1])), 
                                  White()),  #https://gpflow.github.io/GPflow/develop/api/gpflow/kernels/index.html
...
# ------------------------------------------------------------------------------ A graphing module, may result in X I/O error. 
...
"""    
    ...
    # Define the feat and continuous arrays
        ...
        # Plot the results
            plt.scatter(feat, y_array, label=f"(Prior)+Posterior observations, n = {len(x_array)}")
            ...
            plt.title(f"GP Regression; kernel = {self.model.kernel}")
