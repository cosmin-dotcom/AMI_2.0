...
parser.add_argument('-n', type=int, help='Total number of MOFs to screen.', default=650)
parser.add_argument('-r', type=str, help='Ranker to use', default='gp')
parser.add_argument('-p', type=int, help='Load prior.csv dataset - 1 (True) or 0 (False)', default=1)
...
# Save to HDF5
with h5py.File('features_v2.hdf5', 'w') as f:
...
data_manager = InMemoryDataManager.from_indexed_list_in_file("features_v2.csv",
...
if prior_true == 1:
    ...
    X_init, y_init = prior_values['index'].tolist(), prior_values['target'].tolist()
