...
class FileStreamerTruthProvider(ami.abc.TruthProviderInterface):
    ...
    def from_csv_file(cls, path: Union[str, Path], schema: ami.abc.SchemaInterface):
        """AMI reads from CSV file where first column contains file paths"""
        filenames = []
        csv_path = Path(path)
        with csv_path.open(mode="r") as fd:
            ...
            for row in reader:
                if row and len(row) > 0:  # Skip empty rows
                    ...
                    assert p.exists(), f"File does not exist: {p}" # AMI will tell you if you have included a file path but the
                                                                   # file does not exist
                    filenames.append(p)
        return cls(filenames=filenames, _schema=schema)
...
class CsvPersistence:
    ...
    def from_filename(cls, path: Union[str, Path], features_csv_path: Union[str, Path] = None):
        ...
        return cls(writer=writer, feature_headers=feature_headers, feature_data=feature_data)

    def __post_init__(self):
        # Write headers including features for east post-AMI analysis of materials
        if self.feature_headers:
            # Write the column headers
            column_headers = ["index", "value"] + self.feature_headers
    ...
    def append_valid_result(self, index: Index, value):
        # Build output line with features
        output_parts = [str(index), str(value)]
        ...
        if self.feature_data and index < len(self.feature_data):
            ...
            print(f"Index {index} has missing feature data.")
...
class InMemoryDataManager(ami.abc.DataManagerInterface):
    ...
    def from_indexed_list_in_file(cls, 
        ...
        """
        Create from either a text file with paths or a CSV file with features.
